    ...
    <form onSubmit={funcionCotizar} >
      ...
      <select name="propiedad" value={propiedad} onChange={tomarData} >
        ...
        {data.propiedades.map((prop, i) => {
          return (
            <option key={i} className="option" value={prop.factor}  >{prop.tipo}</option>
          );
        })}
      </select>
      ...
      <input list="browses" onChange={tomarData} type="number" name="metros2" min="20" max="500" value={metros2} required />

      <div className="center separador">
        {demora ? <button className="button button-outline"  >Cotizar</button> : <img className="button button-outline" src="/DEMORA.gif" alt="page not found" />}
      </div>
